    ...
    <main>
      <header>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Button id="delete-button" variant="contained" disabled={!todos.some((todo) => todo.checked)} onClick={() => handleDelete()}><DeleteIcon /></Button>
        </form>
      </header>
      <div id="check-all-div">
        <Checkbox id="check-all" disabled={todos.length === 0} checked={allChecked} onChange={(event) => handleCheckAll(event)} />
        <label htmlFor="check-all" className={!todos.length && 'disabled-text'}>{allChecked ? `Uncheck All` : `Check All`}</label>
      </div>
      <ul>
        {todos.map((todo) => (
          <li key={todo.id}>
            <Todo id={todo.id} checked={todo.checked} handleCheckbox={handleCheckbox} value={todo.value} handleValueChange={handleValueChange} />
          </li>
        ))}
      </ul>
    </main>
